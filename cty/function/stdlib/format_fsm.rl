...
func formatFSM(format string, a []cty.Value) (string, error) {
	...
	action err_char {
		return buf.String(), fmt.Errorf("unrecognized format character %q at offset %d", fc, p)
	}
	...
	width = (
		( num $width_num | '*' @width_from_arg ) %has_width
	)?;

	precision = (
		('.' ( digit* $prec_num | '*' @prec_from_arg )) %has_prec
	)?;
